...
BEGIN
    ...
    discount_multiplier := 1 - (SELECT discount_percent FROM promotions WHERE promotion_id = NEW.promotion_id)::DECIMAL / 100;
...
BEFORE INSERT OR UPDATE OF promotion_id ON games
FOR EACH ROW
...
BEGIN
    IF TG_TABLE_NAME = 'games' AND TG_OP = 'UPDATE' AND NEW.genre_ids <> OLD.genre_ids THEN
        ...
        SELECT NEW.game_id, genre_id FROM game_genres WHERE genre_id = ANY(NEW.genre_ids);
    END IF;
...
AFTER UPDATE OF genre_ids ON games
FOR EACH ROW
